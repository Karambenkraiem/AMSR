    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Bienvenue, {user?.prenom} {user?.nom}</p>
        </div>
        <span className="text-sm text-gray-400">{format(new Date(), 'EEEE dd MMMM yyyy', { locale: fr })}</span>
      </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-gray-800">Demandes récentes</h2>
          <Link to="/demandes" className="text-sm text-steg-primary hover:underline">Voir tout →</Link>
        </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {recent.map((d) => {
                  ...
                  >
                    ...
                    <td className="py-3 text-gray-600">{d.chargeTravaux?.prenom} {d.chargeTravaux?.nom}</td>
                    <td className="py-3 text-gray-400">{format(new Date(d.createdAt), 'dd/MM/yyyy')}</td>
                    <td className="py-3">
